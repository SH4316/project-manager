...
발송 대상 채널은 core의 웹 화면 `팀 → 알림 채널`에서 팀 관리자가 등록한다.
이 서비스는 `GET /api/integrations/discord/webhooks?team=<TEAM_ID>`로 그 목록을 읽어
켜져 있는 채널 **전부**로 보낸다(60초 캐시). core를 못 읽으면 마지막 목록, 그것도 없으면
`DISCORD_WEBHOOK_URL`을 쓴다.
...
| `TEAM_ID` | ✅ | — | 알림을 보낼 팀 id |
| `DISCORD_WEBHOOK_URL` | | (빈 값) | 예비 Webhook URL. 발송 대상은 core의 웹 화면 `팀 → 알림 채널`에서 관리하고, core를 못 읽을 때만 이 값을 쓴다 |
...
2. 팀 관리자가 발급한 초대 링크로 그 계정을 팀에 넣고, `팀 → 팀원 관리`에서 역할을 **관리자**로 올린다.
   (Webhook 주소는 비밀이라 팀 관리자만 읽을 수 있다. 토큰은 읽기여도 된다.)
