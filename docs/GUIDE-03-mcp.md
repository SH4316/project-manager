...
`mcp_server/README.md`에 다음 표를 넣고, core의 `/settings/tokens` 화면 안내 문구에도 같은 내용을 쓴다. `<TOKEN>`은 `/settings/tokens`에서 발급한 값. MCP 주소는 `settings.MCP_URL`(환경 변수 `MCP_URL`, 기본 `https://project.sio2.kr`)이 정하고, 화면은 그 값을 박아 그린다.
